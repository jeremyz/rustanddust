...
import ch.asynk.creepingarmor.game.State.StateType;
import ch.asynk.creepingarmor.game.states.StateSelect;
import ch.asynk.creepingarmor.game.states.StateMove;
import ch.asynk.creepingarmor.game.states.StateRotate;
import ch.asynk.creepingarmor.game.states.StatePromote;
import ch.asynk.creepingarmor.game.states.StateEngage;
import ch.asynk.creepingarmor.game.states.StateBreak;
import ch.asynk.creepingarmor.game.states.StateAnimation;
import ch.asynk.creepingarmor.game.states.StateReinforcement;
import ch.asynk.creepingarmor.game.states.StateDeployment;
import ch.asynk.creepingarmor.game.states.StateWithdraw;
...
public class Ctrl implements Disposable
{
    ...
    public Ctrl(final CreepingArmor game, final Battle battle)
    {
        this.game = game;
        this.battle = battle;
    ...
    private StateType actionDone()
    {
        ...
        if (nextState == StateType.DONE) {
            ...
            if (map.activatedUnits.size() > 0) {
                CreepingArmor.debug("Ctrl", "burn down 1AP");
    ...
    public void setState(StateType nextState)
    {
        ...
        CreepingArmor.debug("Ctrl", String.format("  %s -> %s : %s", stateType, nextState, player));

...
public abstract class Map5Marshal extends Map4Orders
{
    ...
    {
        unload(json, "atk", e.attacker, false);
        unload(json, "def", e.defender, false);
        json.writeArrayStart("assists");
        for (Unit u : e.assists)
            unload(json, null, u, false);
        json.writeArrayEnd();
    ...
    {
        JsonValue u = v.get("atk");
        Unit atk = findById(u.getInt("id"));
        u = v.get("def");
        Unit def = findById(u.getInt("id"));

        Order o = Order.get();
        o.setEngage(atk, def);

        u = v.get("dice");
        o.engagement.d1 = u.getInt(0);
        o.engagement.d2 = u.getInt(1);
        o.engagement.d3 = u.getInt(2);
        o.engagement.d4 = u.getInt(3);

        u = v.get("res");
        o.engagement.success = u.getBoolean(0);
        o.engagement.attackSum = u.getInt(1);
        o.engagement.defenseSum = u.getInt(1);

        u = v.get("assists");
        for (int i = 0; i < u.size; i++) {
            JsonValue a = u.get(i);
            Unit unit = findById(a.getInt("id"));
            o.engagement.assists.add(unit);
        }
